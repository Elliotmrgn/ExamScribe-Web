...
class Chapter:
    def __init__(self, doc, chapter_num, chapter_title, start_page_check, end_page_check, total_questions):
        ...
        self.chapter_question_bank = []


    def _extract_page_data(self, page_number):
        return self.doc[page_number].get_textbox((0, 60, self.doc[0].rect.width, self.doc[0].rect.height))

    def _extract_chapter_text(self, start_page, end_page):
        # Builds a string of all text from the chapter
        chapter_text = ""
        for page in range(start_page, end_page+1):
            chapter_text += self._extract_page_data(page)
            chapter_text += '\n'
    ...
    def _extract_questions(self):
        ...
        question_chapter_text = self._extract_chapter_text(self.question_start_page, self.question_end_page)
    ...
    def _extract_answers(self):
        ...
        answer_chapter_text = self._extract_chapter_text(self.answer_start_page, self.answer_end_page)
    ...
    def build_chapter_question_bank(self):
        # Extracts all questions and answers from the chapter
        chapter_questions = self._extract_questions()
        chapter_answers = self._extract_answers()
        ...
        for entry in combined_data:
            ...
            self.chapter_question_bank.append({
    ...
    def _clean_choices(self, choices):
        choices = re.split(r'(^[a-zA-Z]\. +)', choices, flags=re.MULTILINE)
